...
`webdeck` skill without making them the default for every deck.
...
If the wildcard is custom, it must follow WebDeck' no-slop aesthetics:
...
- Keep `webdeck` output as one self-contained HTML file.
