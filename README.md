...
**WebDeck** helps non-designers create beautiful web presentations without knowing CSS or JavaScript. It uses a "show, don't tell" approach: instead of asking you to describe your aesthetic preferences in words, it generates visual previews and lets you pick what you like.
...
```text
/plugin marketplace add https://github.com/LifaSun/WebDeck
```

After that finishes, run:

```text
/plugin install webdeck@webdeck
```

Use the HTTPS URL. The shorter `LifaSun/WebDeck` form may make Claude Code try SSH, which can fail if GitHub is not already in your `known_hosts` file.

Then use it by typing `/webdeck:webdeck` in Claude Code. Claude Code namespaces plugin-installed skills as `/plugin-name:skill-name`.
...
mkdir -p ~/.claude/skills/webdeck/scripts

# Copy the user-facing skill files
cp SKILL.md STYLE_PRESETS.md viewport-base.css html-template.md animation-patterns.md ~/.claude/skills/webdeck/
cp -R bold-template-pack ~/.claude/skills/webdeck/
cp scripts/extract-pptx.py scripts/deploy.sh scripts/export-pdf.sh ~/.claude/skills/webdeck/scripts/
```

Or clone directly:

```bash
git clone https://github.com/LifaSun/WebDeck.git ~/.claude/skills/webdeck
```

Then use it by typing `/webdeck` in Claude Code. Standalone skills are not namespaced.

### Other Coding Agents

Agents such as Codex, Kimi Code, OpenCode, Gemini CLI, or other local coding assistants can use the same core skill. The simplest path is to send the agent this GitHub repo link and ask it to use the WebDeck skill:
...
The Claude Code plugin gives Claude Code a custom marketplace-source install flow and `/webdeck:webdeck` command. Other agents usually do not use that command surface.
...
If installed manually as a standalone Claude Code skill, use `/webdeck` instead.

In non-Claude agents, ask the agent to use the WebDeck skill and point it at this repo or `SKILL.md`.
...
- Claude Code is required only for the custom marketplace-source install and `/webdeck:webdeck` command
